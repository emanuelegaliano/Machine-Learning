...
Il decision boundary di un discrimnatore è la \textbf{frontiera} che separa le diverse classi nello spazio delle caratteristiche. Ovvero quel valore per il classificatore è "indeciso" tra le due classi.
...
\paragraph{Esempio su dataset two moons.}
Si può fare un esempio con AdaBoost su un dataset sintetico \emph{two moons} per visualizzare l'evoluzione del decision boundary al crescere del numero di iterazioni. Come weak learner si può usare un \emph{decision stump} (albero di decisione di profondità 1), che fanno una sola divisione lineare.
