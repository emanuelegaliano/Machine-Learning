...
\subsection{Perchè abbiamo bisogno del Machine Learning}
...
Nel caso del \textbf{Supervised Learning}, l’esperienza coincide con i valori assunti dalle \textbf{random variables}\footnote{In questo contesto, le random variables rappresentano le variabili casuali che descrivono i dati osservati.}. Riprendendo l’esempio precedente, $X$ rappresenta il contenuto della mail e $Y$ l’etichetta associata. La collezione di coppie:
...
\noindent
rappresenta l’esperienza. In questo contesto essa è generalmente vista come una collezione di elementi chiamati \textbf{esempi}. Si tratta tuttavia di un caso particolare, in cui l’interazione del modello è confinata al dataset e le due componenti (input e output) sono fornite simultaneamente.

Se consideriamo invece il caso del \textbf{Reinforcement Learning}, l’esperienza assume una forma più generale: essa coincide con qualunque interazione che il modello ha con l’ambiente. Ad esempio, se il modello sta cercando di uscire da un labirinto, la sua esperienza al tempo $t$ può essere rappresentata dalla tupla:
...
\textit{Ne segue che la definizione formalizzata di esperienza è: l’informazione (storico dei dati oppure interazioni con l’ambiente) a cui il modello è esposto e da cui impara (ossia modifica i suoi parametri interni).}
