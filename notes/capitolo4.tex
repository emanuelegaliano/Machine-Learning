...
\section{Softmax}
I problemi di utilizzare modelli OVA o OVO nella classificazione multiclasse sono diversi:
\begin{itemize}
...
\textbf{N.B.} Ogni classe $k$ ha il proprio vettore di parametri $\vartheta^{(k)}$ (i pesi del modello). Inoltre, una volta finito il learning del modello, la somma delle probabilità restituite dal softmax sarà sempre pari a 1 ($\sum_{k=1}^{K} P_{\vartheta}^{(k)}(y=k | x) = 1$)
