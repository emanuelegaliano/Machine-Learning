...
dove $n$ coincide col numero di strati, o \textit{layer}. Nel caso più semplice questa rete neurale ritorna uno scalare $y \in \mathbb{R}$; in generale l'output è un vettore $y \in \mathbb{R}^k$. Una rete neurale è quindi ottenuta da una \textbf{composizione di funzioni derivabili}. Ogni funzione $f_i$ è detta \textbf{layer}, e andremo a vederne presto alcuni esempi. Ogni layer è composto da un certo numero di \textbf{neuroni}, o unità, che sono a loro volta composti da:
\begin{itemize}
	\item Una trasformazione affine $z = W x + b$, in cui $W$ è la matrice dei pesi, $b$ è il vettore di bias, e $x$ è l'input del layer. Il numero di neuroni di un layer determina il numero di righe della matrice dei pesi $W$.
	\item Una \textbf{funzione di attivazione} $a = \sigma(z)$, che introduce non linearità nella rete. La funzione di attivazione viene applicata elemento per elemento (\textit{element-wise}) al vettore $z$.
\end{itemize}
