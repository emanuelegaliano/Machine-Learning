...
\paragraph{Uso del momentum.} Per risolvere questi problemi, o comunque limitarli, si potrebbe utilizzare la discesa del gradiente con momento, che aiuta a stabilizzare la convergenza e a ridurre l'effetto del rumore nei dati di training. Inoltre, è possibile utilizzare tecniche di regolarizzazione per penalizzare i modelli troppo complessi e prevenire l'overfitting.

\subsection{Parametri di regolarizzazione}

Anche i parametri di regolarizzazione, esattamente come il modello (i suoi parametri), devono essere stabiliti sperimentalmente. Il valore di $\lambda$ influenza molto la regolarizzazione del modello:
\begin{itemize}
	\item Un valore alto di $\lambda$ penalizza maggiormente i pesi del modello, riducendo il rischio di overfitting, ma potrebbe portare a un modello troppo semplice (high bias).
	\item Un valore basso di $\lambda$ consente al modello di adattarsi meglio ai dati di training, ma aumenta il rischio di overfitting (high variance).
\end{itemize} 

Si parte da $\lambda^0 = 0$, testando valori di $\lambda^{(1)}, \lambda^{(2)} \ldots \lambda^{(k)}$ sempre più grandi, verificando come cambia $J_{\text{VAL}}(\vartheta^i)$ in funzione di $\lambda^i$:
\[
\left[
\begin{array}{c}
\lambda^{(0)}\\
...
\paragraph{Interpretazione della procedura.}  
Per ogni valore candidato di $\lambda$, si ottiene un modello diverso (poiché la funzione di costo include il termine di regolarizzazione). Ciascun modello viene valutato sul validation set tramite $J_{\text{VAL}}$, che fornisce una stima della capacità di generalizzazione. Si seleziona quindi il valore di $\lambda$ che minimizza l'errore di validazione. 
...
\subsection{Altre curve di valutazione}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{images/learning_curve_train_validation.png}
...
\begin{itemize}
	...
	\item Presentazione del prof. Giovanni M. Farinella.
\end{itemize}